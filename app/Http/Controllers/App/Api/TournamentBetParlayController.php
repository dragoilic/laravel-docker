...
class TournamentBetParlayController extends Controller
{
    public function post(
        ...
        $request->validate([
            ...
            'pending_odds.*.period' => [
                'required',
                Rule::in(array_values(GamePeriod::toArray())),
            ],
            'wager' => ['required', 'numeric', 'min:100'],
            'pending_odds.*.oddId' => ['required', 'numeric'],
        ]);
        ...
        try {
            ...
            foreach ($request->request->get('pending_odds') as $pendingWager) {
                ...
                $betItems[] = new BetItem($pendingWager['oddId'], $pendingWager['type'], $pendingWager['period'], $tournamentEvent);
            }
