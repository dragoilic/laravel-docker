...
class BookController extends Controller
{
    ...
    public function updateFixture($id)
    {
        ...
        $url = sprintf('%s/api/v1/fixture/%s/update?authtoken=%s', $this->baseUrl, $entity->getApiId(), $this->authToken);
        $response = Http::post($url);
    ...
    public function updateSnapshot($id)
    {
        $entity = $this->entityManager->getRepository(ApiEvent::class)->find($id);
        ...
        $url = sprintf('%s/api/v1/snapshot/%s/update?authtoken=%s', $this->baseUrl, $entity->getApiId(), $this->authToken);
        $response = Http::post($url);
