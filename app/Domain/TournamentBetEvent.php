...
class TournamentBetEvent
{
    ...
    public function __construct(int $eventOddId, string $period, string $type, TournamentEvent $tournamentEvent, TournamentPlayer $tournamentPlayer)
    {
        $odds = $tournamentEvent->getApiEvent()->getOddsById($eventOddId);
        if ($odds === null) {
    ...
    public function getOddType(string $type, string $period): ?string
    {
        $periodTag = null;
        switch($period) {
            ...
            case GamePeriod::SECOND_HALF():
                $periodTag = "secondhalf";
        }
        return ($periodTag) ? $type.'_'.$periodTag : $type;
    }
}
