...
class Tournament
{
    ...
    public function computePlayerRanks(): array
    {
        $sortedPlayers = array_values($this->players->toArray());
        usort($sortedPlayers, fn (TournamentPlayer $a, TournamentPlayer $b) => -$a->getChips() <=> -$b->getChips());
        $rank = 0;
        $rankedPlayers = [];
        foreach ($sortedPlayers as $sortedPlayer) {
            $rank++;
            $playerId = $sortedPlayer->getId();
            $filteredItems = array_filter(
                $this->players->toArray(),
                function ($p) use (&$playerId) {
                    return $p->getId() == $playerId;
                }
            );
            $player = array_values($filteredItems)[0];
            $player->setRank($rank);
