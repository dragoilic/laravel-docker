...
class ApiEvent
{
    ...
    private ?int $sportId;
    /** @ORM\Column(name="league_id", type="integer", nullable=true, options={"unsigned"=true}) */
    ...
    public function getOddsById(int $oddId): ?ApiEventOdds
    {
        $collection = $this->odds->filter(function ($odd) use ($oddId) {
            return $odd->getId() === $oddId;
        });
    ...
    public function getOddsAllLines(): ?Collection
    {
        return $this->odds;
    }

    public function getTotalOddsCount(): int
    {
        return count($this->odds);
    }


    private function updateLine(string $lineId, string $period, string $name, string $type, ?int $price, ?Decimal $line, ?Settlement $settlement): bool
    {
        ...
        if ($lineInstance === null) {
            ...
            $lineInstance = new ApiEventOdds($this, $lineId, $period, $betType, $price, $line);
    ...
    public function updateLines(Line ...$lines): bool
    {
        ...
        foreach ($lines as $line) {
            $updated |= $this->updateLine($line->getId(), $line->getPeriod(), $line->getName(), $line->getType(), $line->getPrice(), $line->getLine(), $line->getSettlement());
        }
